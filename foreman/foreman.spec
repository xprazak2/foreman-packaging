...
Requires: %{?scl_prefix}rubygem(audited) >= 4.3.0
Requires: %{?scl_prefix}rubygem(audited) < 4.6.0
...
Requires: %{?scl_prefix}rubygem(ancestry) < 4.0
Requires: %{?scl_prefix}rubygem(scoped_search) >= 4.1.2
...
BuildRequires: %{?scl_prefix}rubygem(audited) >= 4.3.0
BuildRequires: %{?scl_prefix}rubygem(audited) < 4.6.0
...
BuildRequires: %{?scl_prefix}rubygem(ancestry) < 4.0
BuildRequires: %{?scl_prefix}rubygem(scoped_search) >= 4.1.2
...
BuildRequires: npm(patternfly-react) >= 0.18.2
...
BuildRequires: npm(react-ellipsis-with-tooltip) >= 1.0.3
...
Requires: npm(patternfly-react) >= 0.18.2
...
Requires: npm(react-ellipsis-with-tooltip) >= 1.0.3
...
Requires: %{?scl_prefix_ror}rubygem(execjs) < 3.0.0
...
%changelog
* Tue Jan 30 2018 Ondrej Prazak <oprazak@redhat.com> 1.17.0-0.1.RC1
- Release 1.17.0-RC1
